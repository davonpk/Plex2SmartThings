...
def plexWebHookHandler(){
    ...
    switch(plexJSON?.event) {
        case ["media.play", "media.resume", "media.scrobble"]:
            command = "onplay"; break;
        case "media.pause":
            command = "onpause"; break;
        case "media.stop":
            command = "onstop"; break;
        return
    }
    // send to child apps
    childApps?.each { child ->
...
// Stop running if disable switch is activated
    if (disableSwOn != null) {
        if(disableSwOn?.currentSwitch == "on") { logWriter ("Disabled via switch on"); return }
    }
